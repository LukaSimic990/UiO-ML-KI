...
class InMemoryInvertedIndex(InvertedIndex):
    ...
    def _build_index(self, fields: Iterable[str], compressed: bool) -> None:
        ...
        for doc in self._corpus:
            terms = [] # Since we do not keep track of which field each term occurs in, I collect them all in one list.
            for field in fields:
                it = self._analyzer.terms(doc.get_field(field, None)) # Analyzer returns a generator of normalized terms.
                for item in it:
                    terms.append(item[0])
            counter_dict = Counter(terms) # Counting the occurance of each term in the document.
            for term in counter_dict:
                self._append_to_posting_list(self._add_to_dictionary(term), doc.get_document_id(), counter_dict[term], compressed)
    ...
    def get_document_frequency(self, term: str) -> int:
        term_id = self._dictionary.get_term_id(term)
        if term_id is not None and self._posting_lists[self._dictionary[term]] is not None:
            return self._posting_lists[self._dictionary[term]].get_length()
        else:
            return 0
